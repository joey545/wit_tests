var Microphone, WEBSOCKET_HOST, WitError, log, states;
...
log = /debug/.test(window.location.search) ? (function() {
  ...
  disconnected: {
    connect: function(token) {
      ...
      conn.onopen = (function(_this) {
        return function(e) {
          return conn.send(JSON.stringify(["auth", token]));
        };
      })(this);
...
Microphone.prototype.fsm = function(event) {
  ...
  if (_.isFunction(f)) {
    ...
    this.state = s;
    if (s === 'audiostart' || s === 'audioend' || s === 'ready') {
